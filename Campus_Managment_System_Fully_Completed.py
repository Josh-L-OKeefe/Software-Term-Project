...
# Initialize DB at import time (creates missing tables)
try:
    init_db()
...
class App(ctk.CTk):
    ...
    def forgot_password_page(self):
        ...
        ctk.CTkButton(frame, text="Send Reset (demo)", command=self.reset_password, width=220).pack(pady=12)
        ctk.CTkButton(frame, text="Back", command=self.login_page, width=120, fg_color="gray20").pack(pady=6)
    ...
    def reset_password(self):
        ...
        try:
            ...
            cursor.close()
            cnx.close()
            # For demo: just show a message. In production you'd email a link.
    ...
    def dashboard_page(self):
        ...
        # load user's registered eids into a set for fast lookup
        registered_eids = set()
        try:
            cnx = get_db()
            cursor = cnx.cursor()
        ...
        for evt in events:
            ...
            # compute current registrations count
            try:
                cnx = get_db()
                cursor = cnx.cursor()
    ...
    def register_event(self, eid: int, capacity: int, current_registered: int):
        ...
        try:
            ...
            # likely UNIQUE constraint (user already registered)
    ...
    def user_create_event_page(self):
        ...
        # 🔥 NEW: Load room list from SQL and show dropdown
    ...
    # ADMIN PAGE PLACEHOLDER (not implemented fully)
    def admin_page(self):
        messagebox.showinfo("Admin", "Admin page (not fully implemented).")

    # CLEAR WINDOW UTILITY
